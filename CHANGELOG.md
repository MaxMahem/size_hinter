...
- `ExactSizeIterator` implementation for `InvalidIterator`
- `TestIterator` - iterator that can not be iterated over, but has an arbitrary size hint

### Removed

- Unecessary sealing, and `sealed` crate.

## [0.4.1] - 2026-01-30

### Added

- `SizeHint::lower()` and `SizeHint::upper()` const accessor methods
...
- `InvalidIterator` - iterator that reports an invalid size hint for testing purposes
